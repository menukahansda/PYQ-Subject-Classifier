    ...
    for i, label in enumerate(labels):
        ...
        if label == -1:
            ...
            if i-1 >= 0:
                ...
                if any(img.split('-')[0] == prev.split('-')[0] for img in img_files for prev in prev_img_files):
                    label = newlabel
                    newlabel += 1
            elif i+1 < len(labels):
                next_img_files = txt2img[i+1]
                if any(img.split('-')[0] == nxt.split('-')[0] for img in img_files for nxt in next_img_files):
                    label = newlabel
                    newlabel += 1
            if label == -1:
                continue
        ...
        for img_file in img_files: 
            shutil.move(os.path.join(IMG_FOLDER, img_file), os.path.join(cluster_dir, img_file))
